# Changelog

Все значимые изменения проекта документируются в этом файле.

Формат — [Keep a Changelog 1.0.0](https://keepachangelog.com/ru/1.0.0/),
версионирование — [SemVer](https://semver.org/lang/ru/).

## [1.2.0] — 2026-09-25

**Агенты читают код через индекс: служба спрашивает гард перед `fs_read_file`
и отправляет агента к инструменту code-index там, где индекс отдаёт тот же
файл. Агенты на Codex получили ключ вызова и запираются в каталоге задания
так же, как остальные.**

### Добавлено

- **Гард индекса в `fs_read_file` (`[fs] read_guard`, `read_guard_mcp_prefix`).**
  Перед чтением файла агентом, у которого есть `read_file` code-index, служба
  спрашивает внешнюю программу с договором хуков `PreToolUse` (например
  `code-index-guard` из code-index) и исполняет её ответ: отказ приходит агенту
  ошибкой с подсказкой инструмента индекса. Внешние клиенты и агенты без
  индекса читают как раньше; поломка гарда чтение не ломает. Настройки
  перечитываются без перезапуска.
- **Ключ вызова для агентов на Codex.** Служба передаёт заголовок
  `x-agents-mcp-call` серверу `agents` через `http_headers`, поэтому
  `fs_read_file` у таких агентов заперт в рабочем каталоге задания и проходит
  ту же проверку гардом.
- **`scripts/clean_copy.py`: копия под запретом обычного чтения.** `prepare`
  дописывает каталог копии секцией `[[local]]` в конфиг хука
  `code-index-guard` (`~/.claude/hooks/code-index-guard.toml`,
  переопределяется переменной `CODE_INDEX_GUARD_CONFIG`), `remove` её убирает.
  Копия читается только через свой индекс: `Read`, `Grep`, `cat`/`grep`/`ls`
  по ней хук отклоняет. В общий `daemon.toml` индексатора ничего не
  добавляется, чужие секции конфига не трогаются.

### Совместимость

- Новые настройки `[fs] read_guard` и `read_guard_mcp_prefix` необязательны:
  без них служба ведёт себя как в 1.1.0. Договор инструментов не меняется.

### Проверка

- `cargo fmt --all -- --check`, `cargo clippy --all-targets --all-features -- -D warnings`,
  `cargo test --workspace --all-features`, тесты цепочки — чисто.
- Живьём на службе (Windows): агенты DeepSeek Flash и Sol — файл в индексе →
  отказ с подсказкой, файл вне индекса → чтение, файл вне каталога задания →
  отказ; внешний клиент читает как прежде.
- Стенд на Linux: проверки `mcp_smoke`, `edge_smoke`, `escape_probe`.

## [1.1.0] — 2026-09-21

### Добавлено

- **Провайдер `grok-cli`** — Grok CLI самостоятельным агентом с
  MCP-инструментами. Каждый вызов получает свой временный каталог с
  `.grok/config.toml`: серверы из `[execution] mcp_config` агента, личные
  серверы пользовательского конфига CLI выключены. Встроенные инструменты CLI
  сняты; белый список `[execution] allowed_tools` соблюдается запретом
  остальных инструментов поимённо. Ответ — `--output-format json`, ходы сессии
  CLI пишутся в `agent_turns`. Настройки — секция `[providers.grok_cli]`.
- **Параметр `overrides` у `invoke_agent` и `agent_run`** — перекрытие
  настроек агента на один вызов по закрытому списку ключей: `model.name`,
  `model.temperature`, `model.max_tokens`, `execution.max_turns`,
  `limits.timeout_sec`, `effort`, `cache.enabled`, `mcp.<сервер>.url`.
  Адреса MCP — только уже объявленные агентом серверы и адреса из
  `[agents] allowed_mcp_urls`. Перекрытия входят в ключ кэша, пишутся в
  колонку `agent_calls.overrides` и видны в `agent_history`.
- **Инструменты `chain_cancel` и `task_get`.** `chain_cancel(task_id)`
  отменяет все живые фоновые вызовы задачи и ставит ей статус `cancelled`;
  `agent_cancel` показывает `task_id` в списке живых вызовов.
- **Переключатель `[response] schema_strict`.** При `true` несоответствие
  ответа файлу `response.schema_file` делает вызов неполным и сохраняет сырой
  ответ; по умолчанию поведение прежнее — только предупреждение в журнале.
- **`scripts/clean_copy.py`: несколько копий параллельно.** `prepare` требует
  `--port` и `--alias`, у каждой копии свой каталог индекса и ветка
  `agent/<имя>`; новые команды `commit` и `list`, `remove --all`.
- **CI на GitHub:** на отправку в `main` и запрос слияния — `cargo fmt`,
  `clippy -D warnings`, тесты, сборка выпуска под Linux и Windows.

### Исправлено

- **Разбор ответа в формате JSON больше не обрезается на чужой рамке.**
  Закрывающая рамка искалась первым вхождением, и план с примерами кода внутри
  строкового поля получал статус «неполный». Теперь перебираются кандидаты
  (ответ целиком, тело рамки, кусок по скобкам) с проверкой разбором; при
  полном провале сырой ответ сохраняется в `<runs_dir>/<call_id>-<агент>-raw.txt`.

### Совместимость

- Новые миграции `migrations_pg/007_agent_calls_overrides.sql` и
  `migrations_sqlite/003_agent_calls_overrides.sql` (колонка
  `agent_calls.overrides`); статус задачи `cancelled` в обоих хранилищах.
- `clean_copy.py prepare` без `--port` и `--alias` теперь отказывает —
  значений по умолчанию больше нет.

## [1.0.2] — 2026-09-20

### Исправлено

- **Провайдер codex-cli больше не затирает MCP-серверы из профиля.** Ключ
  `-c mcp_servers={}` подставлялся всегда и обнулял весь список серверов
  профиля `CODEX_HOME`; точечные ключи `mcp_servers.<имя>.url`, добавленные
  следом, его уже не восстанавливали, и агенту на codex нельзя было дать ни
  одного инструмента. Теперь обнуление ставится только тем агентам, которые
  своих серверов не приносят.
- **Провайдер codex-cli передаёт рабочий каталог вызова.** `cwd_template`
  агента не передавался вовсе, и codex работал в каталоге службы. Теперь
  каталог уходит ключом `-C`, и песочница `workspace-write` ограничивает
  запись каталогом задания.
- **Сторож повторов не обрывает ход на одной повторяющейся строке.** Шесть
  вхождений одной длинной строки в пределах 5000 знаков считались петлёй, и
  исполнитель останавливался на первом ходу, описывая структуру с однотипными
  атрибутами `serde`. Теперь порог «подряд» применяется, только если до него
  встретились минимум две разные строки; одиночную строку ловит порог за весь
  ход.

## [1.0.1] — 2026-09-18

### Исправлено

- **Ответ модели с пояснением перед рамкой ```json больше не отвергается.**
  Рамка снималась только с начала ответа, поэтому строка вроде «Проверил три
  места» перед JSON приводила к пометке вызова «неполный», и готовая работа
  агента пропадала (случай повторялся у DeepSeek Flash и у Opus). Теперь рамка
  находится в любом месте ответа; если рамки нет вовсе — берётся кусок от первой
  открывающей скобки до последней закрывающей; если ничего похожего на JSON нет
  — возвращается сам ответ.

## [1.0.0] — 2026-09-13

Первый публичный выпуск: https://github.com/Regsorm/agents-mcp

[1.1.0]: https://github.com/Regsorm/agents-mcp/releases/tag/v1.1.0
[1.0.2]: https://github.com/Regsorm/agents-mcp/releases/tag/v1.0.2
[1.0.1]: https://github.com/Regsorm/agents-mcp/releases/tag/v1.0.1
[1.0.0]: https://github.com/Regsorm/agents-mcp/releases/tag/v1.0.0
